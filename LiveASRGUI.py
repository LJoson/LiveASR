...
app.setObjectName("LiveASRGUIAPP")
...
# print输出重定向到文件
log_f = open('liveasrgui.log', 'w', buffering=1)
sys.stdout = log_f
sys.stderr = log_f

from UI.version import __version__
from UI.util import outputWithDateTime

log_f.write(f"\nliveasr_GUI: {__version__}")
...
# 记录日志
logger_faster_whisper = logging.getLogger("liveasr")
logger_faster_whisper.setLevel(logging.DEBUG)
faster_whisper_logger_handler = logging.FileHandler(r"./liveasr.log", mode="w")
...
from UI.translator import TRANSLATOR, language
...
if __name__ == "__main__":
    ...
    mainWindows.show()  # 先显示主窗口
    pb.setValue(100)

    # 确保主窗口显示后再关闭启动画面
    app.processEvents()  # 处理事件，确保主窗口显示
